...
WHERE inv_id = 10;
-- Use an inner join to select the make and 
-- model fields from the inventory table and 
-- the classification name field from the 
-- classification table for inventory items 
-- that belong to the "Sport" category. 
SELECT i.inv_make,
    i.inv_model,
    c.classification_name
FROM inventory AS i
    JOIN classification AS c ON c.classification_id = i.classification_id
